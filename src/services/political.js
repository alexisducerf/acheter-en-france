...
export const getLegislativesElectionResults2024 = async (code_insee) => {
  const sheetID = "1QQWVVy_Qoes9o-uhJgWulSEm4ElTC0K9wmdwwhyu8bY";
  ...
  try {
    ...
    const result = {
      ...
      participation: {
        inscrits: row.c[4]?.v,
        votants: row.c[5]?.v,
        pourcentage_votants: row.c[6]?.v,
        abstentions: row.c[7]?.v,
        pourcentage_abstentions: row.c[8]?.v,
        exprimes: row.c[9]?.v,
        pourcentage_exprimes_inscrits: row.c[10]?.v,
        pourcentage_exprimes_votants: row.c[11]?.v,
        blancs: row.c[12]?.v,
        pourcentage_blancs_inscrits: row.c[13]?.v,
        pourcentage_blancs_votants: row.c[14]?.v,
        nuls: row.c[15]?.v,
    ...
    // Dynamically parse candidates
    // Each candidate has 9 columns in sequence:
    // N°Panneau, Nuance, Nom, Prénom, Sexe, Voix, % Voix/Ins, % Voix/Exp, Elu
    let currentColumn = 18; // Start after participation data
    const columnsPerCandidate = 9;
    const maxCandidates = 22; // Maximum number of candidates

    for (let i = 0; i < maxCandidates; i++) {
      const baseIndex = currentColumn + (i * columnsPerCandidate);

      // Check if we have valid data for this candidate
      if (row.c[baseIndex + 2]?.v) {
        const candidat = {
          numero_panneau: row.c[baseIndex]?.v,
          nuance: row.c[baseIndex + 1]?.v,
          nom: row.c[baseIndex + 2]?.v,
          prenom: row.c[baseIndex + 3]?.v,
          sexe: row.c[baseIndex + 4]?.v,
          voix: row.c[baseIndex + 5]?.v,
          pourcentage_voix_inscrits: row.c[baseIndex + 6]?.v,
          pourcentage_voix_exprimes: row.c[baseIndex + 7]?.v,
          elu: row.c[baseIndex + 8]?.v === "1"
        };
